...
import { ArrowLeft, MapPin, Briefcase, SlidersHorizontal, X, Check, ChevronLeft, ChevronRight, Clock, Banknote, Search } from "lucide-react"
...
type Filters = { industry: string | null; radius: string; budget: string; urgency: string }
const DEFAULT_FILTERS: Filters = { industry: null, radius: "10", budget: "", urgency: "" }
...
export default function JobsFindMap({ initialJobs, initialCoords, initialPostcode, initialIndustry }: Props) {
  ...
  const refetchJobs = useCallback(async (f: Filters, c: [number, number] = coords) => {
    setLoading(true)
    try {
      const radiusMiles = parseInt(f.radius) || 10
      const latDelta = radiusMiles / 69
      const lngDelta = radiusMiles / (69 * Math.cos((c[0] * Math.PI) / 180))
      ...
      let query = supabase
        ...
        .or(
          `latitude.is.null,and(latitude.gte.${c[0] - latDelta},latitude.lte.${c[0] + latDelta},longitude.gte.${c[1] - lngDelta},longitude.lte.${c[1] + lngDelta})`
        )

      if (f.industry) {
        const kw = f.industry.split(/[\s&,/]+/).find(w => w.length > 3) || f.industry
        query = query.or(`industry.ilike.%${kw}%,industry.is.null`)
      }
      if (f.urgency) query = (query as any).eq("urgency_type", f.urgency)
      if (f.budget === "0-500")   query = (query as any).lte("budget_max", 500)
      else if (f.budget === "500-1k")  query = (query as any).gte("budget_min", 500).lte("budget_max", 1000)
      else if (f.budget === "1k-5k")   query = (query as any).gte("budget_min", 1000).lte("budget_max", 5000)
      else if (f.budget === "5k+")     query = (query as any).gte("budget_min", 5000)
  ...
  const geocodePostcode = async (postcode: string) => {
    ...
    try {
      ...
      if (data?.[0]) {
        ...
        refetchJobs(filters, newCoords)
      }
    } finally { setGeocoding(false) }
  }

  const applyFilters = () => { setFilters(draftFilters); setShowFilters(false); refetchJobs(draftFilters) }
  const clearFilters = () => {
    const r = { ...DEFAULT_FILTERS }
    setDraftFilters(r); setFilters(r); setShowFilters(false); refetchJobs(r)
  }
  const hasActiveFilters = filters.industry !== null || filters.radius !== "10" || filters.budget !== "" || filters.urgency !== ""
  ...
  // Job photo for lightbox (single photo per job)
  ...
  return (
    <div className="fixed inset-0 bg-slate-950">
            ...
            {leafletL && jobsWithCoords.map(job => {
              ...
              return (
                <React.Fragment key={job.id}>
                  {/* Semi-transparent circle for approximate jobs */}
                  {isApprox && (
                    <Circle
                      center={pos}
                      radius={350}
      ...
      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div
        className="fixed top-0 left-0 right-0 flex items-center gap-2 px-3 py-2"
        style={{ zIndex: 20, paddingTop: "max(env(safe-area-inset-top,0px),8px)" }}
      >
        <button onClick={() => router.back()} className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-white shadow-md">
          <ArrowLeft className="w-3.5 h-3.5" />
        </button>
        {editingLocation ? (
          <form onSubmit={e => { e.preventDefault(); geocodePostcode(postcodeInput) }} className="flex-1 flex gap-1.5 items-center">
            <input
              ref={postcodeRef}
              value={postcodeInput}
            ...
            />
            <button type="submit" disabled={geocoding} className="w-7 h-7 rounded-full bg-indigo-500 flex items-center justify-center flex-shrink-0">
            ...
            <button type="button" onClick={() => setEditingLocation(false)} className="w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center flex-shrink-0">
              <X className="w-3 h-3 text-slate-300" />
            </button>
          </form>
        ) : (
          <div
            onClick={() => { setPostcodeInput(locationLabel !== "Nearby" ? locationLabel : ""); setEditingLocation(true); setTimeout(() => postcodeRef.current?.select(), 50) }}
            className="flex-1 min-w-0 flex items-center gap-1.5 bg-slate-800 border border-slate-700 rounded-full px-2.5 py-1.5 shadow-md cursor-pointer hover:border-indigo-500/50 transition-colors"
          >
      ...
      {/* ── Bottom sheet ────────────────────────────────────────────────────── */}
      <div
        className="fixed rounded-t-3xl shadow-2xl border-t border-x border-slate-700/50 flex flex-col overflow-hidden transition-all duration-300 ease-in-out"
        style={{ ...sheetStyle, zIndex: 30, backgroundColor: "#0f172a" }}
      >
        {/* Drag handle / back */}
        <div
          className="flex-shrink-0 flex items-center justify-between px-3 pt-2 pb-1 cursor-grab select-none"
          onClick={() => {
        ...
        {/* ── Job detail (pin clicked) ── */}
        {selectedJob ? (
          <div className="flex-1 min-h-0 overflow-y-auto">

            {/* Photo — clickable to fullscreen if available */}
            {selectedJob.job_photo_url ? (
              <button
                onClick={() => setLightboxIndex(0)}
                className="w-full h-40 overflow-hidden flex-shrink-0 focus:outline-none"
              >
                <img
                  src={selectedJob.job_photo_url}
                  ...
                  <div className="w-7 h-7 rounded-full overflow-hidden bg-slate-700 flex-shrink-0">
                    {selectedJob.homeowner_profiles.profile_photo_url
                      ...
                      : <span className="w-full h-full flex items-center justify-center text-slate-400 text-xs font-bold">
                          {selectedJob.homeowner_profiles.first_name?.[0] ?? "?"}
                        </span>}
                  </div>
            ...
            <div className="flex gap-2 px-4 pb-4 pt-1">
              ...
              {selectedJob.homeowner_profiles?.user_id && (
                <button
                  onClick={() => router.push(`/messages/${selectedJob.homeowner_profiles!.user_id}`)}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-2 pb-2 space-y-1">
              {loading ? (
                ...
                <div className="text-center py-6">
                  <p className="text-slate-400 text-xs">No jobs found nearby. Try a wider radius.</p>
                </div>
              ) : jobs.map(job => (
                <div
                  key={job.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {job.urgency_type === "asap" && (
                        <span className="text-[10px] font-medium text-red-400">Urgent</span>
                      )}
      ...
      {showFilters && (
        <div className="fixed inset-0" style={{ zIndex: 50 }}>
          ...
          />
          <div
            className="absolute rounded-3xl shadow-2xl border border-slate-700/60 flex flex-col overflow-hidden"
            style={{ top: HEADER, bottom: "max(env(safe-area-inset-bottom,0px),10px)", left: "12px", right: "12px", backgroundColor: "#0f172a", zIndex: 51 }}
          >
            {/* Title */}
                    ...
                    >
                      <span className={`w-6 h-6 rounded-md flex items-center justify-center flex-shrink-0 ${isAll ? "bg-slate-700" : (active ? "bg-white/10" : style!.iconBg)}`}>
                        {isAll
                          ...
                          : Icon && <Icon className={`w-3 h-3 ${active ? "text-indigo-300" : style!.iconColor}`} />}
                      </span>
                ...
                <div className="flex gap-1.5">
                  {RADIUS_OPTIONS.map(opt => (
                    <button key={opt.value} onClick={() => setDraftFilters(f => ({ ...f, radius: opt.value }))}
                      className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${draftFilters.radius === opt.value ? "bg-indigo-500 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}
                    >{opt.label}</button>
                  ))}
                </div>
              </div>
