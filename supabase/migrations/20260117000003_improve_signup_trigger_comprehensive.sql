-- Improved Signup Trigger with Comprehensive Orphaned Record Cleanup
-- Date: 2026-01-17
-- Description: Enhanced trigger to handle all edge cases for re-signup after account deletion
...
BEGIN
  ...
  BEGIN
    ...
    IF user_type = 'homeowner' OR is_homeowner THEN
...
COMMENT ON FUNCTION public.handle_new_user IS
  'Improved trigger function that automatically creates user and profile records when a new auth user is created. Comprehensively handles re-registration by cleaning up ALL orphaned records from previous deleted accounts. Ensures idempotent operations with ON CONFLICT clauses.';
