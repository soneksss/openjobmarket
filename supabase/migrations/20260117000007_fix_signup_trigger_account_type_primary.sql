-- Fix Signup Trigger - Use account_type as primary decision factor
-- Date: 2026-01-17
-- Description: Create profile based on account_type (individual vs company), not role flags
...
BEGIN
  ...
  -- Handle experience_level as TEXT (will be cast to enum type if needed during INSERT)
  ...
  RAISE NOTICE 'Account type: %, User type: %', account_type, user_type;
  RAISE NOTICE 'Roles - Jobseeker: %, Homeowner: %, Employer: %, Tradespeople: %',
    is_jobseeker, is_homeowner, is_employer, is_tradespeople;
  ...
  -- =====================================================================
  -- CREATE ONE PROFILE BASED ON ACCOUNT_TYPE
  -- One email = One account = One profile type
  ...
  BEGIN
    ...
    -- Companies, Self-employed, Agencies → company_profiles
    IF account_type = 'company' THEN
      ...
      RAISE NOTICE 'Created company profile for % (Business account)', NEW.email;

    -- INDIVIDUAL ACCOUNTS (account_type = 'individual')
    -- Jobseekers, Homeowners (or both) → professional_profiles or homeowner_profiles
    ELSE
      -- For jobseekers/professionals, create professional_profiles
      IF is_jobseeker OR user_type IN ('professional', 'jobseeker') THEN
        ...
        RAISE NOTICE 'Created professional profile for % (Individual account - Jobseeker)', NEW.email;

      -- For homeowners only, create homeowner_profiles
      ELSIF is_homeowner OR user_type = 'homeowner' THEN
        ...
        RAISE NOTICE 'Created homeowner profile for % (Individual account - Homeowner only)', NEW.email;

      -- Fallback: Create professional profile for individual accounts
      ELSE
        INSERT INTO public.professional_profiles (
          user_id,
          first_name,
        ...
        RAISE NOTICE 'Created default professional profile for % (Fallback)', NEW.email;
      END IF;
    END IF;
  EXCEPTION WHEN OTHERS THEN
...
COMMENT ON FUNCTION public.handle_new_user IS
  'FIXED: Signup trigger using account_type as primary decision factor. One email = One account = One profile type. Business accounts (company) → company_profiles. Individual accounts (individual) → professional_profiles or homeowner_profiles. Provides defaults for NOT NULL fields. Handles orphaned records for re-signup.';
