-- Migration: Fix Email Conflict on Re-registration
-- Date: 2026-01-09
-- Description: Handle cases where user deletes account but email still exists in public.users

-- First, let's delete orphaned records (users without auth records)
-- This cleans up any existing orphaned data from deleted accounts
DELETE FROM public.users
WHERE id NOT IN (SELECT id FROM auth.users);

-- Now update the trigger to handle both id and email conflicts
...
BEGIN
  ...
  -- Check if a user with this email already exists (from a previously deleted account)
  SELECT id INTO existing_user_id
  FROM public.users
  WHERE email = NEW.email AND id != NEW.id
  LIMIT 1;

  -- If orphaned record exists, delete it first
  IF existing_user_id IS NOT NULL THEN
    -- Delete related profiles first
    DELETE FROM public.professional_profiles WHERE user_id = existing_user_id;
    DELETE FROM public.company_profiles WHERE user_id = existing_user_id;
    DELETE FROM public.homeowner_profiles WHERE user_id = existing_user_id;
    -- Delete the orphaned user record
    DELETE FROM public.users WHERE id = existing_user_id;
  END IF;

  -- Create public.users record (or update if somehow still exists with same id)
...
COMMENT ON FUNCTION public.handle_new_user IS
  'Trigger function that automatically creates user and profile records when a new auth user is created. Handles re-registration by cleaning up orphaned records first.';
