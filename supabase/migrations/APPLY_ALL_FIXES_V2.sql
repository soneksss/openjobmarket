-- =====================================================
-- COMPREHENSIVE FIX FOR OPENJOBMARKET DATABASE V2
-- =====================================================
-- This script combines all necessary fixes:
-- 1. Cross-role job applications (companies can apply to tasks)
-- 2. Message trigger field name fix (receiver_id -> recipient_id)
--
...
-- Step 5: Drop ALL existing RLS policies (must be outside DO block)
...
DROP POLICY IF EXISTS "Professionals can apply to jobs" ON public.job_applications;
DROP POLICY IF EXISTS "Companies can apply to tasks" ON public.job_applications;

DO $$
BEGIN
    RAISE NOTICE '✓ Dropped all old RLS policies';
END $$;
