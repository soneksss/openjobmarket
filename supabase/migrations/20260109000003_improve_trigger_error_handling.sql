-- Migration: Improve Trigger Error Handling
-- Date: 2026-01-09
-- Description: Remove broad exception handler to see actual errors, add specific error handling
...
BEGIN
  ...
  BEGIN
    ...
    IF existing_user_id IS NOT NULL THEN
      RAISE NOTICE 'Found orphaned user record with email %, cleaning up', NEW.email;
      ...
      RAISE NOTICE 'Cleanup complete for orphaned user %', existing_user_id;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Error during orphaned record cleanup: %', SQLERRM;
    -- Continue anyway - this shouldn't block new user creation
  END;
  ...
  RAISE NOTICE 'Successfully created user and profile records for %', NEW.email;
  RETURN NEW;
END;
$$;
...
COMMENT ON FUNCTION public.handle_new_user IS
  'Trigger function that automatically creates user and profile records when a new auth user is created. Handles re-registration by cleaning up orphaned records. Uses specific error handling instead of broad exception catching.';
