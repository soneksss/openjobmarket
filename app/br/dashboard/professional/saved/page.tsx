...
export default async function SavedJobsPageBR() {
  try {
    console.log("[SAVED-JOBS-PAGE-BR] Page loading...")
    ...
    try {
      ...
      console.error("[SAVED-JOBS-PAGE-BR] Error getting user:", error)
    }

    if (!user) {
      console.log("[SAVED-JOBS-PAGE-BR] No user found, redirecting to login")
      redirect("/auth/login?locale=pt-BR&returnUrl=/br/dashboard/professional/saved")
    }

    console.log("[SAVED-JOBS-PAGE-BR] User authenticated:", user.email)
  ...
  try {
    ...
    if (profileError) {
      console.error("[SAVED-JOBS-PAGE-BR] Error fetching profile:", profileError)
    } else {
      profile = profileData
    }
    ...
    if (profile) {
      ...
      if (savedJobsError) {
        console.error("[SAVED-JOBS-PAGE-BR] Error fetching saved jobs:", savedJobsError)
      } else {
    ...
    console.error("[SAVED-JOBS-PAGE-BR] Unexpected error:", error)
  ...
  return (
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <Button
            ...
            asChild
          >
            <Link href="/br/dashboard/professional">
                      ...
                      {job.company_profiles?.logo_url ? (
                        ...
                        <Avatar className="h-16 w-16">
                          <AvatarFallback className="bg-blue-100 text-blue-600 font-bold text-lg">
                            {job.company_profiles?.company_name?.substring(0, 2).toUpperCase() || 'CO'}
                          </AvatarFallback>
                        </Avatar>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" asChild>
                          <Link href={`/br/company/${job.company_profiles?.user_id}`}>
                            View Company
                          </Link>
                        </Button>
                      </div>
    ...
    console.error("[SAVED-JOBS-PAGE-BR] Fatal error:", error)
    return (
