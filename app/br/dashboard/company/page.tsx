...
export default async function CompanyDashboardPageBR() {
  console.log("[COMPANY-BR] Company dashboard page loading...")
  ...
  if (!user) {
    console.log("[COMPANY-BR] No user found, redirecting to login")
    redirect("/auth/login?locale=pt-BR&returnUrl=/br/dashboard/company")
  }

  console.log("[COMPANY-BR] User found:", user.id)
  ...
  if (profileError) {
    console.log("[COMPANY-BR] Profile error:", profileError)
  }

  if (!profile) {
    console.log("[COMPANY-BR] No company profile found, redirecting to onboarding")
    redirect("/br/onboarding")
  }

  console.log("[COMPANY-BR] Company profile found:", profile.company_name)

  // Check if profile is complete and valid
  const isProfileComplete = profile.company_name && profile.industry && profile.location

  if (!isProfileComplete) {
    console.log("[COMPANY-BR] Incomplete company profile detected, redirecting to onboarding to complete setup")
    redirect("/br/onboarding")
  }
  ...
  if (reviewsError) {
    console.error("[COMPANY-BR] Error fetching reviews:", reviewsError)
  }
  ...
  console.log("[COMPANY-BR] Jobs found:", jobs?.length || 0)
  ...
  if (countsError) {
    console.log("[COMPANY-BR] Application counts error:", countsError)
  }
  ...
  // Get recent applications RECEIVED for jobs posted by this company
  ...
  if (applicationsError) {
    console.log("[COMPANY-BR] Received applications error:", applicationsError)
  }

  console.log("[COMPANY-BR] Received applications found:", receivedApplications?.length || 0)
  ...
  if (submittedError) {
    console.log("[COMPANY-BR] Submitted applications error:", submittedError)
  }

  console.log("[COMPANY-BR] Submitted applications found:", submittedApplications?.length || 0)
  ...
  // Enrich RECEIVED applications with job and applicant details
  ...
  console.log("[COMPANY-BR] Rendering dashboard with data:", {
