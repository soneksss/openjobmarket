...
export default async function SavedJobsPage() {
  try {
    console.log("[SAVED-JOBS-PAGE] Page loading...")
    ...
    try {
      ...
      console.log("[SAVED-JOBS-PAGE] Auth check result:", { authUser: !!authUser, authError })
      if (!authError && authUser) {
        user = authUser
      }
    } catch (error) {
      console.error("[SAVED-JOBS-PAGE] Error getting user:", error)
    }

    if (!user) {
      console.log("[SAVED-JOBS-PAGE] No user found, redirecting to login")
      redirect("/auth/login")
    }

    console.log("[SAVED-JOBS-PAGE] User authenticated:", user.email)
  ...
  try {
    ...
    if (profileError) {
      console.error("[SAVED-JOBS-PAGE] Error fetching profile:", profileError)
    } else {
      profile = profileData
    }
    ...
    if (profile) {
      ...
      if (savedJobsError) {
        console.error("[SAVED-JOBS-PAGE] Error fetching saved jobs:", savedJobsError)
      } else {
    ...
    console.error("[SAVED-JOBS-PAGE] Unexpected error:", error)
  ...
  return (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" asChild>
                          <Link href={`/company/${job.company_profiles?.user_id}`}>
                            View Company
                          </Link>
                        </Button>
                      </div>
    ...
    console.error("[SAVED-JOBS-PAGE] Fatal error:", error)
    return (
